...
当前 Planner 是确定性实现，最多生成两个并行查询；结果不足时允许一次基于现有领域和偏好的 Replan，之后必须澄清或拒答。跨节点依赖 DAG、并发上限和节点级停止条件已经实现；尚未接入外部 Web Search Tool。后续通过统一 `web_search` 适配器接入成熟外部搜索服务，完成来源和 Prompt Injection 安全评审前保持关闭。
