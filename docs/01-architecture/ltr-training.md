...
本增量把已完成归因的曝光和行为事件转换为可重复构建的排序评测样本，并使用固定时间边界切分开发、验证和测试集。现有 Python 类型保留 `LTR` 前缀以避免破坏兼容性，但后续用途调整为比较 RRF、开源 Cross-Encoder 和在线轻量重排服务，不训练、发布或加载 LTR 模型。
...
- `src/seekora_agent/domain/training.py`：定义版本化排序特征、评测样本和时间切分结果；当前 `LTR` 类名前缀属于兼容名称；
...
- `src/seekora_agent/application/exposure.py`：登记曝光时从最终结果复制并排序召回源分数，供后续排序评测重放。

## 6. 当前边界

- 尚未提供生产数据仓库读取器和评测集文件导出任务；
- 尚未做位置倾向估计、负样本降采样和跨用户/会话泄漏审计；
- 技术路线已明确不进行 LTR 模型训练；后续直接评测开源 Cross-Encoder 或在线轻量重排服务；
- 数据导出和评测作业必须只读取仍具备合法处理依据且未被隐私删除的数据。
